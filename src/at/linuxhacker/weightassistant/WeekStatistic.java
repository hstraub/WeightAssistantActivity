...
public class WeekStatistic {
	List<MeasuringPoint> weekPoints = new ArrayList( );
	int weekOfYear = -1;
	double min = 0;
	double max = 0;
	double sum = 0;
	...
	WeekStatistic( MeasuringPoint point ) throws Exception {
	...
	public void addMeasurePoint( MeasuringPoint point ) throws Exception {
		...
		this.weekPoints.add( point );
		this.sum += point.getWeight( );
		this.average = this.sum / this.weekPoints.size( );
